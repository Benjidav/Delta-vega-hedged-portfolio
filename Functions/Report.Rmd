...
![](C:\Users\benja\OneDrive\Images\Saved Pictures\vega.png)
...
Besides, **we have another set of three parameters** : $q, l$ and $q'$ corresponding to the quantities of stock, cash and K6.5 option. We have to solve the following linear system to find the value of $q, q'$ and $q''$ :
...
In a first while, let's compute the **implied volatilities** of K6 and K6.5 over time and give an overview of them thanks to optimization algoritms (Newton-Raphson & bisection method).
...
```
Now, let's compute the $\nu_t$ of the two options (thanks to the formula mentioned above) and get an overview of them.
...
head(vega_Opt6.5, 10)
```
Then, we have to compute the ratio of volatility increments $$\frac{\partial \sigma'}{\partial \sigma} = 
\frac{\sigma'_{t+1} - \sigma'_t}{\sigma_{t+1} - \sigma_t}$$. As a reminder, we have to consider the following formula 
...
Now, let's compute the value of $q, l$ and $q'$ and get an overview of them (reminder : the first three values can't be computed) :
...
Finally, we use the previous parameters and the iteration process mentioned above in order to build the history of the portfolio replication : 
...
pf.deltaVega_hedging = function (price_option_1, price_option_2, stock_price, list_prm_vega)
{
  ...
  j = 1
  #V0 = C0 en v̩rifiant que la premiere valeur est pas NA
  ...
  for (i in (j + 1):length(portfolio))
  {
    ...
    {
      portfolio[i] = portfolio[i - 1] + list_prm_vega[i - 1, 3] * (stock_price[i] - stock_price[i - 1]) + list_prm_vega[i - 1, 1] * (price_option_2[i] - price_option_2[i - 1])
    }
  }
  return (portfolio)
}
...
pf_DeltaVega_Replication_Opt6_graphe
```
